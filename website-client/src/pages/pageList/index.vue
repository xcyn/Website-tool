<template>
  <div class="PageContainer">
    <el-button class="addBtn" type="primary" @click="handleAdd"
      >新增页面</el-button
    >
    <el-table border :data="pages" stripe style="width: 100%">
    ...
    <el-dialog :visible.sync="visible" width="30%" center>
      <div style="text-align: center">
        <div>用手机扫描可以查看效果, 二维码只保存3分钟哦</div>
        <img :src="qrImg" alt="" />
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    async getPageList() {
      ...
      const pages = (
        await axios.get(
          `${baseUrl}/pageConfig/getPages?currentPage=${currentPage}&pageSize=${pageSize}`
        )
      ).data.data;
    ...
    async handleSee(record) {
      ...
      const qrCode = (
        await axios.get(`${baseUrl}/pageConfig/qrCode?url=${pageSrc}`)
      ).data.data;
    ...
    handleDel(record) {
      this.$confirm("确定要删除?", "提示", {
        ...
        .then(async () => {
          await axios.delete(
            `${baseUrl}/pageConfig/deletePages?pageId=${record.pageId}`
          );
          this.$message({
            type: "success",
            message: "删除成功",
      ...
      this.$prompt("请输入页面名称", "提示", {
        ...
        .then(async ({ value }) => {
          try {
            ...
            const pageId = (await axios.get(`${baseUrl}/pageConfig/getPageId`))
              .data.data;
