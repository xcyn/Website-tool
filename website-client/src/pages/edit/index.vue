  ...
  <div>
    <el-row :gutter="24">
      <el-col :span="3" class="edit-col">
        <div class="title">组件区</div>
        <div class="componentList">
          <div
            class="component"
            @dblclick="handleSlectComponent(component)"
            :key="key"
            v-for="(component, key) in componentList"
          >
            <el-button
              ><i class="el-icon-star-on"></i>{{ component.message }}</el-button
            >
          </div>
          <el-alert
            class="alert"
      ...
      <el-col :span="3" class="edit-col">
        <div class="title">布局区域</div>
        <div class="edit-layout">
          <div
            class="component"
            :key="key"
            v-for="(component, key) in pageconfigList"
          >
            <div>
              <el-button @click="handleSlectLayout(component)"
                ><i class="el-icon-school"></i
                >{{ component.message }}</el-button
              >
            </div>
            <div v-if="component.id === selectComponent.id">
              ...
              <el-button
                ...
                size="mini"
                type="success"
                @click="handleMoveDown(component)"
              >
              ...
              <el-form :model="pageCommonData" class="demo-form-inline">
                <div :key="key" v-for="(config, key) in pageCommonConfig">
                  <el-form-item :label="config.message">
                    <el-input
                      v-model="pageCommonData[config.name]"
                      placeholder="请输入"
                    ></el-input>
                  </el-form-item>
                </div>
                <el-form-item>
                  <el-button type="primary" @click="handlePageSubmit"
                    >提交页面配置数据</el-button
                  >
                </el-form-item>
              </el-form>
                ...
                <div :key="key" v-for="(config, key) in selectComponent.schema">
                  <el-form-item :label="config.label">
                    ...
                    <el-input
                      ...
                      v-model="componentsDatas[selectComponent.id][key]"
                      placeholder="请输入"
                    ></el-input>
                  </el-form-item>
                </div>
                <el-form-item>
                  <el-button type="primary" @click="handleComponentSubmit"
                    >提交组件配置数据</el-button
                  >
                </el-form-item>
              </el-form>
  ...
  methods: {
    ...
    async getComponentList() {
      const res = await axios.get(
        `${baseUrl}/pageConfig/getPageConfList?pageId=${this.pageId}`
      );
    ...
    async getPageCpDatas() {
      const res = await axios.get(
        `${baseUrl}/pageConfig/getPageCpDatas?pageId=${this.pageId}`
      );
    ...
    async getPageCommonConfig() {
      const res = await axios.get(
        `${baseUrl}/pageConfig/getPageCommonConfig?pageId=${this.pageId}`
      );
    ...
    async getPageCommonData() {
      const res = await axios.get(
        `${baseUrl}/pageConfig/getPageCommonData?pageId=${this.pageId}`
      );
    ...
    async getComponentDatas() {
      const res = await axios.get(
        `${baseUrl}/pageConfig/getPageComponentDatas?pageId=${this.pageId}`
      );
    ...
    async savePageCpDatas(params = {}) {
      await axios.post(
        `${baseUrl}/pageConfig/savePageCpDatas?pageId=${this.pageId}`,
        {
          pageId: this.pageId,
          component: params,
        }
    ...
    async updatePageCpDatas(params = {}) {
      await axios.put(
        `${baseUrl}/pageConfig/updatePageCpDatas?pageId=${this.pageId}`,
        {
          pageId: this.pageId,
          component: params,
        }
    ...
    async handlePageSubmit() {
      await axios.put(
        `${baseUrl}/pageConfig/updatePageCommonData?pageId=${this.pageId}`,
        {
          pageId: this.pageId,
    ...
    async handleComponentSubmit() {
      await axios.put(
        `${baseUrl}/pageConfig/updatePageComponentData?pageId=${this.pageId}`,
        {
          pageId: this.pageId,
    ...
    handleSlectComponent(component) {
      component.id = `pageId_${this.pageId}__${Math.floor(
        +Math.random() * 1000
      )}`;
